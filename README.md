...
Windows / Android 上，云端凭证**不进 git**，走本机 `Resources/Private/SupabaseSettings.asset`（仓库只留 `SupabaseSettings.example.asset`）。
...
>
> 此分支上 `UXPref.SupabaseUrl` / `SupabaseKey` 固定为空、`HasSupabaseConfig` 为 false，**不读取**本机 `SupabaseSettings.asset`（否则填好的本地文件会打进 WebGL 包）。签到与进度走 privateKV，不因缺 Supabase 配置而关闭。
> `SupabaseSaveUploader.cs` 已删除（内含凭据字面量的死代码）。

---

## Addressables 约定

- 组：`Units`、`BGM`、`Visuals`、若干 `Localization-*`。Windows / Android 上 BGM 组必须 **Include in Build**；本 WebGL 分支为 **0**（BGM 由宿主播放，不打进 Player）。
